...
```{r}
#lapply ggplot to Ozone and Solar.R columns of each name
lapply(names(by_month), function(x) {
  ggplot(by_month[[x]], aes(Ozone, Solar.R)) +
    geom_point() +
    ggtitle(bquote(paste("Relationship Between Ozone density and Solar radiation \nfor month"~ .(x))))
 }
  )
